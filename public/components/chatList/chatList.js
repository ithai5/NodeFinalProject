            ...
            .then((chats) => {
                chats.chats.forEach((chat) => {
                    const recipientId = chat.users.filter(
                        (user) => user !== chats.userId
                    )[0] //to get the name of the user that the sessionUser chats with
                    fetch('/api/users/' + recipientId)
                        .then((results) => results.json())
                        .then((user) => {
                            user = user.user
                            const recipient = createDivTag(
                                'a',
                                'recipient',
                                user.firstName + ' ' + user.lastName
                            ) //creating div element and a tag with the link to the chat page
                            if (roomsNotifications.includes(chat.id)) {
                                //recipient.classList.add("has-notification"); //if notificaion and room has the same id
                                const notification = createDivTag(
                                    'span',
                                    'notification-mark',
                                    ''
                                )
