...
fetch("/api/users/profile").then(result => result.json()).then(user => {
    roomsNotifications = user[0].notifications.map(room => room.room); //getting the user rooms with notifications
    fetch("/api/chats").then(results => results.json()).then(chats => {
        chats.chats.forEach(chat => {
            ...
            fetch("/api/users/" + recipientId ).then(results => results.json()).then(user => {
                const recipient = createDivTag("a", "recipient", user[0].firstName + " " + user[0].lastName); //creating div element and a tag with the link to the chat page
