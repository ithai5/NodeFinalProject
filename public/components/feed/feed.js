...
let endPoint
switch (
    pageTitle[1] //check the url to know which list of post need to be requested from the db
) {
    case 'search':
...
fetch(endPoint)
    ...
    .then((feed) => {
        ...
        if (feed.length === 0) {
            alert(
                "Sorry, but we couldn't find what you have been looking for Pupsi :-("
            )
        }
